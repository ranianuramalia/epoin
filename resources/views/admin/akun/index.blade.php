...
<body>
    ...
    <a href="{{ route('logout') }}" onclick="event.preventDelfault(); document.getElementById('logout-form').submit();">logout</a>
    <br><br>
    ...
    <table class="tabel">
        ...
        <tr>
            ...
            <td>
                <a href="{{ route('akun.edit', $user->id)}}" class="btb btn-sm btn-primary">EDIT</a>
            </td>
            @else
            <td>
                @if($user->usertype == 'siswa')
                <form onsubmit="return confirm('jika akun siswa di hapus maka data siswa akan ikut terhapus,apakah anda yakin?');" action="{{route('akun.destroy',$user->id)}}" method ="POST">
                    @else
